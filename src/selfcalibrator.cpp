...
#define DEBUG_SELF_CALIBRATOR 1
...
void SelfCalibrator::Calibrate(const vector<Mat>& imgs, Matx33d& int_mat) {
    ...
    initial_int_mat = Matx33d(2500, 0, 2016, 0, 2500, 1512, 0, 0, 1);
...
void SetVertexPairs(vector<pair<vector<Point2f>, vector<Point2f>>> &vertices_pair) {
    // pair.first: 4 * Point2f
    // pair.second: 4 * Point2f
    pair<vector<Point2f>, vector<Point2f>> temp01;
    //1.jpg clockwise
    temp01.first.push_back(Point2f(1113.0, 198.0));
    temp01.first.push_back(Point2f(3252.0, 165.0));
    temp01.first.push_back(Point2f(2865.0, 1431.0));
    temp01.first.push_back(Point2f(81.0, 1584.0));
    //2.jpg clockwise
    temp01.second.push_back(Point2f(1197.0, 351.0));
    temp01.second.push_back(Point2f(3333.0, 162.0));
    temp01.second.push_back(Point2f(3141.0, 1491.0));
    temp01.second.push_back(Point2f(330.0, 1899.0));
    vertices_pair.push_back(temp01);

    pair<vector<Point2f>, vector<Point2f>> temp12;
    //2.jpg clockwise
    temp12.first.push_back(Point2f(1197.0, 351.0));
    temp12.first.push_back(Point2f(3333.0, 162.0));
    temp12.first.push_back(Point2f(3141.0, 1491.0));
    temp12.first.push_back(Point2f(330.0, 1899.0));
    //3.jpg clockwise
    temp12.second.push_back(Point2f(1476.0, 261.0));
    temp12.second.push_back(Point2f(3663.0, 438.0));
    temp12.second.push_back(Point2f(3054.0, 1674.0));
    temp12.second.push_back(Point2f(274.0, 1467.0));
    vertices_pair.push_back(temp12);

    pair<vector<Point2f>, vector<Point2f>> temp02;
    //1.jpg clockwise
    temp02.first.push_back(Point2f(1113.0, 198.0));
    temp02.first.push_back(Point2f(3252.0, 165.0));
    temp02.first.push_back(Point2f(2865.0, 1431.0));
    temp02.first.push_back(Point2f(81.0, 1584.0));
    //3.jpg clockwise
    temp02.second.push_back(Point2f(1476.0, 261.0));
    temp02.second.push_back(Point2f(3663.0, 438.0));
    temp02.second.push_back(Point2f(3054.0, 1674.0));
    temp02.second.push_back(Point2f(274.0, 1467.0));
    vertices_pair.push_back(temp02);

}

void setAllVertices(vector<vector<Point2f>> &all_vertices)
{
    all_vertices.clear();

    // 1.jpg
    vector<Point2f> temp;
    temp.push_back(Point2f(1132.0, 248.0));
    temp.push_back(Point2f(81.0, 1584.0));
    temp.push_back(Point2f(2865.0, 1431.0));
    all_vertices.push_back(temp);

    // 2.jpg
    temp.clear();
    temp.push_back(Point2f(1192.0, 352.0));
    temp.push_back(Point2f(330.0, 1899.0));
    temp.push_back(Point2f(3140.0, 1492.0));
    all_vertices.push_back(temp);

    // 3.jpg
    temp.clear();
    temp.push_back(Point2f(1476.0, 261.0));
    temp.push_back(Point2f(274.0, 1467.0));
    temp.push_back(Point2f(3024.0, 1664.0));
    all_vertices.push_back(temp);
}
...
void SelfCalibrator::EnumerateParallelogramProjectionMat(
    ...
    //SetVertexPairs(img_vertices_pairs);
    printf("Vertex Size: %lu\n", img_vertices_pairs.size());
    printf("Found vertex pair: \n");
    cv::Mat draw = imgs[0];
    cvtColor(draw, draw, CV_GRAY2RGB);

#if DEBUG_SELF_CALIBRATOR != 0
    for (int i = 0; i < img_vertices_pairs.size(); i++) {
        pair<vector<Point2f>, vector<Point2f>> temp = img_vertices_pairs[i];
        auto first_pair = temp.first;
        auto second_pair = temp.second;

        for (int j = 0; j < first_pair.size() ; j++) {
            std::cout  << j + 1 << ". first pair: " << first_pair[j] << "\n";
            std::cout << j + 1 << ". second pair: " << second_pair[j] << "\n\n";

            cv::Point2f p = first_pair[j];
            circle(draw, p, 30, Scalar(255, 0, 0), -1);

            cv::Point2f p2 = second_pair[j];
            circle(draw, p2, 30, Scalar(0, 255, 0), -1);
        }
    }
    imwrite("/home/louis/self_calib_projects/camera-self-calibration/bin/output_imgs/vertices.png", draw);
#endif
    ...
    for (size_t i = 0; i < imgs.size() - 1; ++i) {
        for (size_t j = i + 1; j < imgs.size(); ++j) {
            ...
            if (para_projs.size() <= j) {
                ...
                orb_detector->detectAndCompute(img1, noArray(), kps1, descs1);
                orb_detector->detectAndCompute(img2, noArray(), kps2, descs2);
...
void SelfCalibrator::FindParallelogramVertices(
    ...
    auto detector = AKAZE::create();
    auto orb_detector = ORB::create();
    auto matcher = BFMatcher(NORM_HAMMING, true);
    ...
    Mat descs1;
    Mat img_t;
    //detector->detectAndCompute(img1, Mat(), kps1, descs1);
    orb_detector->detectAndCompute(img1, noArray(), kps1, descs1);
    ...
    for (size_t i = 1; i < imgs.size(); ++i) {
        ...
        vector<KeyPoint> kps2;
        Mat descs2;
        //detector->detectAndCompute(img2, Mat(), kps2, descs2);
        orb_detector->detectAndCompute(img2, Mat(), kps2, descs2);
        ...
        drawMatches(img1, kps1, img2, kps2, matches, resultImg,
	    Scalar::all(-1), Scalar::all(-1),vector<char>(), 
        DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);

        //printf("IMGS SIZE: %lu\n", imgs.size());
        std::string save_name = "/home/louis/self_calib_projects/camera-self-calibration/bin/output_imgs/" + std::to_string(img_count + 1) + ".jpg";
        cv::imwrite(save_name, resultImg);
        img_count++;
    ...
    for (size_t i = 0; i < imgs.size() - 1; ++i) {
        for (size_t j = i + 1; j < imgs.size(); ++j) {
            ...
            Mat secondResultImg;
            drawMatches(img1, kps1, img2, kps2, matches, secondResultImg,
            Scalar::all(-1), Scalar::all(-1),vector<char>(), 
            DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
            std::string save_name = "/home/louis/self_calib_projects/camera-self-calibration/bin/output_imgs/second_" + std::to_string(second_count + 1) + ".jpg";
            cv::imwrite(save_name, secondResultImg);
            second_count++;
    ...
    for (size_t i = 0; i < all_vertices.size(); ++i) {
        ...
        line(img, vs.at(2), vs.at(0), Scalar(0, 0, 255), 5);
        line(img, vs.at(0), vs.at(1), Scalar(0, 0, 255), 5);
        line(img, vs.at(1), vs.at(2), Scalar(0, 0, 255), 5);

        putText(img, "A", vs.at(0), FONT_HERSHEY_SIMPLEX, 5.0,
                Scalar(0, 0, 255), 8);
        putText(img, "B", vs.at(1), FONT_HERSHEY_SIMPLEX, 5.0,
                Scalar(0, 0, 255), 8);
        putText(img, "C", vs.at(2), FONT_HERSHEY_SIMPLEX, 5.0,
                Scalar(0, 0, 255), 8);

        std::cout << "\nVERTEX A: " << vs.at(0) << "\n";
        std::cout << "VERTEX B: " << vs.at(1) << "\n";
        std::cout << "VERTEX C: " << vs.at(2) << "\n\n";

        std::string name = "/home/louis/self_calib_projects/camera-self-calibration/bin/output_imgs/bebug_" + std::to_string(pers_count + 1) + ".jpg";
        cv::imwrite(name, img);
        pers_count++;
        /*
        stringstream title;
